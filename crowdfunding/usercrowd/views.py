...
def register_user(request):
    if request.method == 'POST':
        ...
        if(request.POST['password'] == request.POST['repassword']):
            ...
            newuser =Usercrowd.objects.create(first_name=request.POST['first_name'], last_name=request.POST['last_name'],
                                   email=request.POST['email'],password=request.POST['password'],phone_number=request.POST['phone_number'],profile_img=request.FILES['profile_img'],expire_date=datetime.datetime.now() + datetime.timedelta(days=1))
